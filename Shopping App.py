...
class ECommerceApp:
    def __init__(self):
        ...
        # Welcome Message
        print(color.BOLD + color.RED + "Welcome to the KKUBS Store.com !!!" + color.END + " \n\n\n ")
    ...
    def create_demo_data(self):
        ...
        product1 = Product(1, "Soccer Boots", "footwear", 50, 10)
        product2 = Product(2, "Rain Strom Waterproof Suit", "clothing", 80, 7)
        product3 = Product(3, "MENS Jackets", "clothing", 110, 10)
        product4 = Product(4, "Tucker Caps", "clothing", 20, 18)
        product5 = Product(5, "HP Laptop EliteBook", "electronics", 820, 7)
        product6 = Product(6, "Mens Pants", "Clothing", 80, 13)
        product7 = Product(7, "USSSSB Bluetooth speaker", "electronics", 120, 7)
        product8 = Product(8, "men Slippers", "footwear", 70, 23)
        product9 = Product(9, "MESSI Footwear", "footwear", 150, 21)
        product10 = Product(10, "NIGGAS Shoes", "footwear", 110, 19)
    ...
    def user_login(self, username, password):
        if username in self.users and self.users[username].password == password:
            self.users[username].session_id = "{}{:3d}{:0005d}".format(username, random.randint(1, 999),
                                                                       random.randint(1, 1110))
    ...
    def display_cart_menu(self, user):
        while True:
            ...
            for i in user.cart:
                item_list.append(i)
                item += 1
                print(item, ". ", user.cart[i]["name"], " ----- $", user.cart[i]["price"], " ------ QTY: ",
                      user.cart[i]["quantity"], "---->TOTAL :\t\t$", (user.cart[i]["price"] * user.cart[i]["quantity"]))
    ...
    def display_catalog(self, user, option):
        ...
        while True:
            ...
            if (option == 1 or option == 0 or option == -1):
                ...
                for prod in self.catalog.products:
                    ...
                    if (option == -1):  # Option=-1 Full Catalog for Admin
                        print(
                            "{}. PID:  {} --- P_NAME:  {} --- P_CATEGORY: {} --- P_PRICE: ${} --- P_QTY: {}  ".format(i,
                        ...
                        print("\t {}.".format(i), self.catalog.products[prod].name, " ---------- $ ",
                              self.catalog.products[prod].price)
            ...
            if (display_option >= 1 and display_option <= i):
                ...
                print("{} --- QTY {} Added to {} Cart ".format(
                    self.catalog.products.get(list_in_Menu[display_option - 1]).name, qty, user.username))
...
if (app.user_login(user_name, password) == "USER"):
    ...
    while True:
        ...
        if (option == 9999):  # Exit out of Shopping
            ...
            print(
                color.BOLD + color.RED + "Thank you for shopping with the KKUBS Stores.com !!!\n Have a Nice Day !!!" + color.END + " \n\n\n ")
            break
...
# Admin Login to Manage Inventory
    ...
    while True:
        if (app.admin_menu(user1) == 9999):
            print(
                color.BOLD + color.RED + "Thank you Admin for updating the shopping Inventory. Hope you enjoyed the Demo Marketplace Dhukan.com !!!\n Have a Nice Day !!!" + color.END + " \n\n\n ")
            break
else:
